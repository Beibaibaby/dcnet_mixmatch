...
for calibration_loss_wt in 1; do
  for model in occam_resnet18_v2_k9753_weighted_poe_detach; do
    for gamma in 1.0 2.0 3.0; do
      for main_loss in CELoss; do
        for calibration_loss in MDCALoss; do
          ...
          optimizer=${optim} \
          expt_suffix=gamma_${gamma}_${main_loss}_${calibration_loss}_${calibration_loss_wt}_subset_${subset_percent}_prec_${precision}
        done
      done
    done
  done
