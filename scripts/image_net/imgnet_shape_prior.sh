...
for main_loss in CELoss; do
  for model in occam_resnet18_v2_k9753_same_width; do
    for fa_loss_wt in 0.01; do
      for da_loss_wt1 in 0.01; do
        for da_loss_wt2 in 0.01; do
...
for main_loss in CELoss; do
  for model in occam_resnet18_v2_k9753_same_width; do
    for fa_loss_wt in 0.1; do
      for da_loss_wt1 in 0.1; do
        for da_loss_wt2 in 0.1; do
          for blur_sigma in 0.1; do
            CUDA_VISIBLE_DEVICES=${GPU} python main.py \
            model.name=${model} \
            trainer=shape_prior_trainer \
